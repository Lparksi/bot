...
async def _(session: NoticeSession):
    if session.event.group_id in GROPUS:
        if session.event.sub_type == 'ban':
            ...
            await session.send(f"""用户[CQ:at,qq={user_id}]被禁言[CQ:at,qq={duration}]秒,执行人：[CQ:at,qq={duration}]""")
    else:
        pass
